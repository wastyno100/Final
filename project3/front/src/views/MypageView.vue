...
const mypageList = (no) => listNo.value = no;
...
// 현재 페이지 감지해서 세션에 저장
watch(listNo, () => {
  sessionStorage.setItem('mypagesCate', listNo.value)
})

onMounted(() => {
  // 페이지 돌아왔을때 세션에 저장되어있는 페이지 가져옴
  listNo.value = JSON.parse(sessionStorage.getItem('mypagesCate')) || 1
})
</script>
      ...
      <v-row>
        ...
        <v-col cols="2" class="d-flex justify-end">
          <v-card class="elevation-2 rounded-lg text-center" max-width="100" height="190">
            <v-list nav dense>
              <v-list-item-group v-for="item in mypages" v-bind:key="item.no">
                <v-list-item @click="mypageList(item.no)" class="mb-2" :class="{ 'selected': listNo === item.no }">
